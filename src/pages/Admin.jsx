  ...
  return (
    <div>
      ...
      <div className="w-full h-auto text-center md:py-8 lg:py-10 text-white bg-black">
        ...
        <h2>Food Items</h2>
        <button onClick={toggleNewItemFields} className="my-2">New Item</button>
        {showNewItemFields && (
          <div className="py-4">
            <input className="m-2" type="text" placeholder="New Item Name" value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} />
            <input className="m-2" type="text" placeholder="Price" value={newItem.price} onChange={e => setNewItem({ ...newItem, price: e.target.value })} />
            <input className="m-2" type="text" placeholder="Image URL" value={newItem.image_url} onChange={e => setNewItem({ ...newItem, image_url: e.target.value })} />
            <button onClick={addFoodItem} className="m-2">Add</button>
          </div>
        )}
        {foodItems.map((item) => (
          <div key={item.id} className="py-2 flex items-center justify-center">
            {editMode[item.id] ? (
              <>
                ...
                <input type="text" value={item.image_url} onChange={e => updateField(item.id, 'image_url', e.target.value)} />
              </>
            ) : (
              <div className="flex items-center">
                <span className="mx-2">{item.name} - {item.price}</span>
                <img src={item.image_url} alt={item.name} className="w-16 h-16 object-cover mx-2" />
              </div>
            )}
            <button onClick={() => {
              ...
              className="mx-2"> {editMode[item.id] ? 'Save' : 'Modify'}
            </button>
            <button onClick={() => deleteFoodItem(item.id)} className="mx-2">Delete</button>
